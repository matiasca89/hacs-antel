...
class MockPage:
    ...
    async def wait_for_selector(self, selector, **kwargs): pass

    async def content(self):
        return HTML_SAMPLE
        
    async def inner_text(self, selector):
        # Return content from HTML_SAMPLE. In real Playwright, hidden elements
        # might be omitted, but for testing logic we want to ensure it works
...
async def run_test():
    ...
    try:
        ...
        if data.billing_period == "1 de enero al 31 de enero":
            print("✅ Billing Period extracted correctly")
        else:
